...
`node`/`bun` (or the `@frontal-labs/vibe-cli`) and build it for production with `vibe/build`.
...
The `@frontal-labs/vibe-cli` (a TypeScript CLI) wraps the same flow for convenience once the
agentic layer lands.
