...
describe("scaffold", () => {
  it("renders the project template to a target directory", () => {
    ...
    try {
      ...
      const mainVibe = readFileSync(join(root, "app", "main.vibe"), "utf8")
      expect(mainVibe).toContain('config { name "my-app"')
      const pkg = JSON.parse(readFileSync(join(root, "app", "package.json"), "utf8")) as {
        name: string
      }
      expect(pkg.name).toBe("my-app")
      expect(existsSync(join(root, "app", "vibe.config.ts"))).toBe(true)
