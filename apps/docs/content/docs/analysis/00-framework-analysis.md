...
description: "A package-by-package assessment of what exists in `packages/` **today**. This is"
---
...
- **`ask()` is a deliberate stub.** It throws `notImplementedError` pointing at
  "Phase 4 (Models)." This is intentional and documented as the 🔴 headline item in
  the [audit](./03-current-state-audit.md#-the-headline-api-is-unimplemented) — the
  framework has no product value until the [agentic implementation plan](../plan/02-agentic-implementation-plan.md)
  lands. Everything above exists to make removing this stub straightforward.
